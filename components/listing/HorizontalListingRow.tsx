...
export default function HorizontalListingRow({ title, listings }: Props) {
  ...
  const isFavorite = listing.isFavorite || Math.random() < 0.4;

  // Generate a random rating between 3.5 and 5.0 with 1 decimal point
  const fakeRating = (Math.random() * 1.5 + 3.5).toFixed(1);
  ...
  return (
    ...
    >
      ...
      <div className="p-3 space-y-0.5">
        ...
        <p className="text-yellow-500 text-sm">★ {parseFloat(rating).toFixed(2)}</p>
      </div>
    </Link>
  );
})}
